...
def retrieveFiles(emotion):
    trainingFiles = glob.glob("D:\\University\\Dissertation\\Database\\testingDataset\\%s\\*" %emotion)
    predictingFiles = glob.glob("D:\\University\\Dissertation\\Database\\completeDave\\%s\\*" %emotion)
    print(predictingFiles)
    random.shuffle(trainingFiles)
    training = trainingFiles
    #[:int(len(trainingFiles)*0.8)]
    predicting = predictingFiles
    #[-int(len(trainingFiles)*0.2):]
